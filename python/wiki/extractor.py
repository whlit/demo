...
def toPage(etree: Element):
    ...
    for e in etree:
        ...
        elif tag == 'ns': page.ns = e.text
        elif tag == 'id': page.id = e.text
...
def toRevision(etree: Element):
    ...
    for e in etree:
        tag = e.tag
        if tag == 'id': revision.id = e.text
        elif tag == 'parentid': revision.parentid = e.text
...
def toText(etree: Element):
    ...
    text.size = etree.attrib.get('bytes')
    return text